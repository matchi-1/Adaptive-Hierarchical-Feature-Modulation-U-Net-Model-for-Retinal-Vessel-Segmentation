...
class BasicConv
    Purpose:
        Implementation helper (not in paper, but in repo). 
        Convenience block for Conv2d with optional BatchNorm and ReLU.

    Parameters:
        in_planes (int):            Number of input channels C_in.
        out_planes (int):           Number of output channels C_out.
        kernel_size (int or tuple): Conv kernel size. (7 is used in SpatialGate)
    ...
    Notes:
        ...
        - BN and ReLU can be disabled via flags; 
        - this class is used by SpatialGate() to build its 7×7 conv.
'''
...
# used only in ChannelGate's MLP
...
class ChannelGate (Channel Attention Module; CAM)
    Purpose (from paper):
        ...
        Use both global average-pooling and global max-pooling, each passed through a shared MLP, 
        summed, then squashed.
...
class CBAM
    ...
    Parameters:
        ...
        pool_types (list[str]): CAM pooling types; paper uses ['avg','max']. Extras ['lp','lse'] available in authors' repo but removed here.
