...
class DPCNStackedInput_MATHFI_Timm(nn.Module):
    ...
    def __init__(
        ...
        # --- DPCN → encoder-level projections (M2Net-style left branch) ---
        in_ch_dpcn = self.T * self.Cd  # T iterations × Cd channels

        self.dpcn_to_s1 = nn.Conv2d(in_ch_dpcn, C1, kernel_size=1, bias=True)
        self.dpcn_to_s2 = nn.Conv2d(in_ch_dpcn, C2, kernel_size=1, bias=True)
        self.dpcn_to_s3 = nn.Conv2d(in_ch_dpcn, C3, kernel_size=1, bias=True)
        self.dpcn_to_s4 = nn.Conv2d(in_ch_dpcn, C4, kernel_size=1, bias=True)
    ...
    def forward(self, x1chw: torch.Tensor, fov: torch.Tensor | None = None) -> torch.Tensor:
        ...
        ys = self.dpcn(x1chw, fov=fov)   # expects DPCN_snake forward(x, fov=None)
        ...
        # --- M2-style: inject DPCN stack into all encoder levels ---
        # Resize feats to each encoder stage and project to its channels
        dpcn_s1 = self.dpcn_to_s1(F.interpolate(
            feats, size=s1.shape[-2:], mode="bilinear", align_corners=False
        ))
        dpcn_s2 = self.dpcn_to_s2(F.interpolate(
            feats, size=s2.shape[-2:], mode="bilinear", align_corners=False
        ))
        dpcn_s3 = self.dpcn_to_s3(F.interpolate(
            feats, size=s3.shape[-2:], mode="bilinear", align_corners=False
        ))
        dpcn_s4 = self.dpcn_to_s4(F.interpolate(
            feats, size=s4.shape[-2:], mode="bilinear", align_corners=False
        ))

        # Fuse with encoder features (additive; you can switch to concat+conv if you want)
        s1 = s1 + dpcn_s1
        s2 = s2 + dpcn_s2
        s3 = s3 + dpcn_s3
        s4 = s4 + dpcn_s4
