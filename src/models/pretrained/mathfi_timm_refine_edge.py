...
class FinalRefine(nn.Module):
    ...
    def __init__(self, in_ch: int, *, use_spatial_cbam: bool = True):
        ...
        # optional tiny residual CBAM to precision-filter fused feat
    ...
    def forward(self, feat: torch.Tensor):
        ...
        # amplify centerline responses, lightly suppress “interior thickness” via edge
        ...
        # scale ≥ 1, larger → softer; edge lowers scale so edges are crisper
...
class MATHFI_TimmEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.asfg_d4 = AdaptiveSelectiveFusionGate(
            ...
            tau=1.0,                                  # lower τ → more decisive weights
            prior_logits=(-0.6, +0.0, +1.2),          # bias toward CBAM at finest scale
            edge_boost_gain=0.0,                      # keep 0 to avoid MSU “edge-thickening”
            agree_boost_gain=0.8                      # agreement → favor CBAM cleanup
        )
    ...
    def forward(self, x1chw):
        ...
        # keep output size aligned (already matched to x1chw via earlier upsampling if needed)
        ...
        return {
            "logits": logits_refined,           # final segmentation logits
            "edge_logits": edge_logits,         # auxiliary supervision (boundaries)
            "skel_logits": skel_logits          # auxiliary supervision (centerlines)
        }
