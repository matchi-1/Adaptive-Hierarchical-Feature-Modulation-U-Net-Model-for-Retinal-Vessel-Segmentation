...
class MATHFI_TimmEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.enc_cbam = nn.ModuleList([
            ResidualCBAM(C1, reduction=cbam_reduction, use_spatial=True,  alpha_init=0.20),  # s1: allow spatial to clean rims
            ResidualCBAM(C2, reduction=cbam_reduction, use_spatial=False, alpha_init=0.15),  # s2: channel-only
            ResidualCBAM(C3, reduction=cbam_reduction, use_spatial=False, alpha_init=0.15),  # s3: channel-only
            ResidualCBAM(C4, reduction=cbam_reduction, use_spatial=False, alpha_init=0.10),  # s4: channel-only, very light
        ])
