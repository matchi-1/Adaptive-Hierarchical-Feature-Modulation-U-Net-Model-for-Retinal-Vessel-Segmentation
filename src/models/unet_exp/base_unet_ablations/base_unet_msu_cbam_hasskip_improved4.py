...
class UNetWithMSU_HASSkip_CBAM_ASFG(nn.Module):
    ...
    def __init__(self, in_channels: int = 1, cbam_reduction: int = 16):
        ...
        # CBAM on each encoder output and at bottleneck (channel→spatial)
        self.cbam_e1 = CBAM(  64, reduction_ratio=cbam_reduction, use_spatial=True)
        self.cbam_e2 = CBAM( 128, reduction_ratio=cbam_reduction, use_spatial=True)
        self.cbam_e3 = CBAM( 256, reduction_ratio=cbam_reduction, use_spatial=True)
        self.cbam_e4 = CBAM( 512, reduction_ratio=cbam_reduction, use_spatial=True)
        self.cbam_b  = CBAM(1024, reduction_ratio=cbam_reduction, use_spatial=True)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # encoders
        s1, p1 = self.e1(x);   s1 = self.cbam_e1(s1)
        s2, p2 = self.e2(p1);  s2 = self.cbam_e2(s2)
        s3, p3 = self.e3(p2);  s3 = self.cbam_e3(s3)
        s4, p4 = self.e4(p3);  s4 = self.cbam_e4(s4)
        
        #bottleneck
        b = self.bottleneck(p4); b = self.cbam_b(b)
